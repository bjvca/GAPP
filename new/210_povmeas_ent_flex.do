...
gen     triwt = 0  
replace triwt = 11 - round(50*abs(cons_pc_tpi/food_povline_ent-1)+0.5) if abs(cons_pc_tpi/food_povline_ent-1)<=0.2 


	gen tripopwt=triwt*hhweight*hhsize 

	collapse (mean) nf_pc_nom food_povline_ent [aw=tripopwt] , by(spdomain) 
...
*	lab var fdshr_t1 "year t1 (e.g. 1996) poverty line food share" 
	...
	lab var fdshr "year t2 (e.g. 2002) poverty line foo so that mean (and total) nominal consumption equals
* mean (and total) spatially deflated consumption.d share" 
